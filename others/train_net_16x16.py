...
y_probabilty, y_predict, y_one_hot, total_loss_16x16, accuracy_16x16, learning_rate_current, train_step, opt_vars_all, \
    opt_vars_res4 = net.net_16x16(x, y, qp, global_step, LEARNING_RATE_INIT, DECAY_RATE, DECAY_STEP)
...
for i in range(ITER_TIMES):
    ...
    with tf.device('/gpu:0'):
        _, learning_rate, loss, accuracy = sess.run([train_step, learning_rate_current, total_loss_16x16, accuracy_16x16],feed_dict={x: images_input, y: lable_input, qp: qp_input,global_step: feed_step})
    ...
    if step % ITER_TIMES_PER_EVALUATE == 0:
        ...
        for j in range(TIMES_PER_COUNT_ACCURACY):
            ...
            # 验证时不更新网络参数
            loss, accuracy = sess.run([total_loss_16x16, accuracy_16x16],feed_dict={x: images_input, y: lable_input, qp: qp_input, global_step: feed_step})
    ...
    if step % ITER_TIMES_PER_SAVE_MODEL == 0:
        saver.save(sess, 'Models'+str(CU_WIDTH) +'x'+ str(CU_HEIGHT)+'/ing/model_16x16_%d.dat'%step)
        saver_res4.save(sess, 'Models'+str(CU_WIDTH) +'x'+ str(CU_HEIGHT)+'/ing/model_res4_%d.dat' % step)

saver.save(sess, 'Models/models_'+str(CU_WIDTH) +'x'+ str(CU_HEIGHT)+'/final/model_16x16.dat')
saver_res4.save(sess, 'Models'+str(CU_WIDTH) +'x'+ str(CU_HEIGHT)+'/final/model_res4.dat')

...
y_probabilty, y_predict, y_one_hot, total_loss_8x4, accuracy_8x4, learning_rate_current, train_step, opt_vars_all, \
    opt_vars_res6 = net.net_8x4_16x4_32x4(x, y, qp, min_RDcost, RDcost, global_step, LEARNING_RATE_INIT, DECAY_RATE, DECAY_STEP)


######################     feed dict       ###########################
saver = tf.train.Saver(opt_vars_all, write_version=tf.train.SaverDef.V2)
saver_res6 = tf.train.Saver(opt_vars_res6, write_version=tf.train.SaverDef.V2)
...
for i in range(ITER_TIMES):
    ...
    with tf.device('/gpu:0'):
        _, learning_rate, loss, accuracy = sess.run([train_step, learning_rate_current, total_loss_8x4, accuracy_8x4],feed_dict={x: images_input, y: lable_input, qp: qp_input,min_RDcost: min_RDcost_input,RDcost: RDcost_input,global_step: feed_step})
    ...
    if step % ITER_TIMES_PER_EVALUATE == 0:
        ...
        for j in range(TIMES_PER_COUNT_ACCURACY):
            ...
            # 验证时不更新网络参数
            loss, accuracy = sess.run([total_loss_8x4, accuracy_8x4],feed_dict={x: images_input, y: lable_input, qp: qp_input,min_RDcost: min_RDcost_input,RDcost: RDcost_input, global_step: feed_step})
            loss_8x4_list.append(loss)
            accuracy_8x4_list.append(accuracy)

        loss = sum(loss_8x4_list[0:TIMES_PER_COUNT_ACCURACY]) / TIMES_PER_COUNT_ACCURACY
        accuracy = sum(accuracy_8x4_list[0:TIMES_PER_COUNT_ACCURACY]) / TIMES_PER_COUNT_ACCURACY

...
y_probabilty, y_predict, y_one_hot, total_loss_8x8, accuracy_8x8, learning_rate_current, train_step, opt_vars_all, opt_vars_res5 \
    = net.net_8x8_16x8_32x8(x, y, qp, min_RDcost, RDcost, global_step, LEARNING_RATE_INIT, DECAY_RATE, DECAY_STEP)


######################     feed dict       ###########################
saver = tf.train.Saver(opt_vars_all, write_version=tf.train.SaverDef.V2)
saver_res5 = tf.train.Saver(opt_vars_res5, write_version=tf.train.SaverDef.V2)
...
for i in range(ITER_TIMES):
    ...
    with tf.device('/gpu:0'):
        _, learning_rate, loss, accuracy = sess.run([train_step, learning_rate_current, total_loss_8x8, accuracy_8x8],feed_dict={x: images_input, y: lable_input, qp: qp_input,min_RDcost: min_RDcost_input,RDcost: RDcost_input,global_step: feed_step})
    ...
    if step % ITER_TIMES_PER_EVALUATE == 0:
        ...
        for j in range(TIMES_PER_COUNT_ACCURACY):
            ...
            # 验证时不更新网络参数
            loss, accuracy = sess.run([total_loss_8x8, accuracy_8x8],feed_dict={x: images_input, y: lable_input, qp: qp_input,min_RDcost: min_RDcost_input,RDcost: RDcost_input, global_step: feed_step})
            loss_8x8_list.append(loss)
            accuracy_8x8_list.append(accuracy)

        loss = sum(loss_8x8_list[0:TIMES_PER_COUNT_ACCURACY]) / TIMES_PER_COUNT_ACCURACY
        accuracy = sum(accuracy_8x8_list[0:TIMES_PER_COUNT_ACCURACY]) / TIMES_PER_COUNT_ACCURACY
    ...
    if step % ITER_TIMES_PER_SAVE_MODEL == 0:
        ...
        saver_res5.save(sess, 'Models/models_'+str(CU_WIDTH) +'x'+ str(CU_HEIGHT)+'/ing/model_res5_%d.dat' % step)

saver.save(sess, 'Models/models_'+str(CU_WIDTH) +'x'+ str(CU_HEIGHT)+'/final/model_'+str(CU_WIDTH) +'x'+ str(CU_HEIGHT)+'.dat')
saver_res5.save(sess, 'Models/models_'+str(CU_WIDTH) +'x'+ str(CU_HEIGHT)+'/final/model_res5.dat')

...
y_probabilty, y_predict, y_one_hot, total_loss_8x8, accuracy_8x8, learning_rate_current, train_step, opt_vars_all, opt_vars_res5 \
    = net.net_8x8_16x8_32x8(x, y, qp, global_step, LEARNING_RATE_INIT, DECAY_RATE, DECAY_STEP)
...
for i in range(ITER_TIMES):
    ...
    with tf.device('/gpu:0'):
        _, learning_rate, loss, accuracy = sess.run([train_step, learning_rate_current, total_loss_8x8, accuracy_8x8],feed_dict={x: images_input, y: lable_input, qp: qp_input,global_step: feed_step})
    ...
    if step % ITER_TIMES_PER_EVALUATE == 0:
        ...
        for j in range(TIMES_PER_COUNT_ACCURACY):
            ...
            # 验证时不更新网络参数
            loss, accuracy = sess.run([total_loss_8x8, accuracy_8x8],feed_dict={x: images_input, y: lable_input, qp: qp_input, global_step: feed_step})
